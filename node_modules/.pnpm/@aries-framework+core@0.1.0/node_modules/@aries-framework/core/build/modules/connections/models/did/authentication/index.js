...
function AuthenticationTransformer() {
    return class_transformer_1.Transform(({ value, obj, type, }) => {
        // TODO: PLAIN_TO_PLAIN
        if (type === class_transformer_1.TransformationType.PLAIN_TO_CLASS) {
            return value.map((auth) => {
                ...
                if (auth.publicKey) {
                    ...
                    const publicKey = class_transformer_1.plainToInstance(publicKeyClass, publicKeyJson);
                    ...
                    const publicKey = class_transformer_1.plainToInstance(publicKeyClass, auth);
                    return new EmbeddedAuthentication_1.EmbeddedAuthentication(publicKey);
                }
            });
        }
        else {
            return value.map((auth) => (auth instanceof EmbeddedAuthentication_1.EmbeddedAuthentication ? class_transformer_1.instanceToPlain(auth.publicKey) : auth));
        }
    });
}
